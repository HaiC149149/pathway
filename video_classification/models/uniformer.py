...
def FeedForward(dim, mult=4, dropout=0.):
    return nn.Sequential(LayerNorm(dim), nn.Conv3d(dim, dim * mult, 1),
...
class GlobalMHRA(nn.Module):
    ...
    def forward(self, x):
        ...
        q, k, v = map(lambda t: rearrange(t, 'b (h d) n -> b h n d', h=h),
                      (q, k, v))
...
class Uniformer(nn.Module):
    def __init__(self,
        ...
        self.to_tokens = nn.Conv3d(channels,
                                   init_dim, (3, 4, 4),
                                   stride=(2, 4, 4),
                                   padding=(1, 0, 0))
        ...
        self.to_logits = nn.Sequential(Reduce('b c t h w -> b c', 'mean'),
                                       nn.LayerNorm(last_dim),
                                       nn.Linear(last_dim, num_classes))

    def forward(self, video):
        batch_size, depth, channels, h_x, w_x = video.shape
        video = video.view(batch_size, channels, depth, h_x, w_x)
